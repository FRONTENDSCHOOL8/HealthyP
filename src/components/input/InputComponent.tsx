...
export default function InputComponent({option, placeholder, bgColor} : InputProps) {
  ...
  function pwConfirm(e: React.ChangeEvent<HTMLInputElement>) {
    if(e.target.value === password) {
      ...
      setConfirmBorder('border-warning border-2'); 
    }
  }
  ...
  function validatePassword(e : React.ChangeEvent<HTMLInputElement>) {
    if(!pwReg(e.target.value)) {
      ...
      setPwBorder('border-warning border-2');
  ...
  const inputMappings = {
    search: (
      <>
        ...
        <input id="search-input" className={`w-full h-36pxr py-0 px-10pxr bg-gray_150 ${bgColor} rounded-md outline-primary`} type="text" placeholder="재료, 해시태그, 요리로 검색해주세요" />
      </>
    ),
    email: (
      <>
        <label htmlFor="email-input" className="text-gray-500 text-foot-em">이메일
          <input id="email-input" type="email" className="w-full h-48pxr py-0 px-10pxr bg-gray_150 rounded-md outline-primary" placeholder="이메일을 입력해주세요" />
        </label>
      </>
    ),
    password: (
      <>
        <label htmlFor="password-input" className="text-gray-500 text-foot-em">비밀번호
          ...
          type="password" 
          className={`w-full h-48pxr py-0 px-10pxr bg-gray_150 rounded-md ${pwBorder} outline-primary`} 
    ...
    passwordConfirm: (
      <>
        <label htmlFor="password-confirm-input" className="text-gray-500 text-foot-em">비밀번호 확인
          ...
          type="password" 
          className={`w-full h-48pxr py-0 px-10pxr bg-gray_150 rounded-md ${confirmBorder} outline-primary`} 
          placeholder="비밀번호를 입력해주세요" 
          onChange={pwConfirm}/>
